...
def process_file(file_path, smoothing_radius = 2, smoothing = False, downsample_radius = None):

    grid_spacings = [2,4,8,16,32]
    output_path = file_path.parent
    results = {}
    results['Sample'] = file_path.name
    print(f"Processing: {file_path.name}\n")
    seg = sitk.ReadImage(file_path)
    # print(seg.GetSpacing(), seg.GetSize())
    ...
    if downsample_radius:
        ...
        dss_filter.SetOutputSpacing(
            [x * downsample_radius for x in [0.8, 0.8, 0.8]])  # sct resolution hardcoded to be 0.8 microns isotropic

        dss_filter.SetOutputOrigin(seg.GetOrigin())
        seg = dss_filter.Execute(seg)
    else:
        seg.SetSpacing([0.8, 0.8, 0.8])
    ...
    lacunarity_value = Lacunarity.Lacunarity(ccmp_villae, grid_spacings)

    results["Lacunarity"] = sum(lacunarity_value)/len(grid_spacings)

    return results

"""This script was designed as a proof of concept for gross statistics taken from segmented placental microstructure, it
# was developed using the segmented datasets from Mary"""
if __name__ == '__main__':
    # FILE I/O parameters
    file_path = r"U:\Consolidator Grant Data\MicroCT\MicroCT Punch Datasets\Normal\JT21032P1_Normal_5.5um\JT21032P1_CubeSegmentation\JT21032P1Segmentation_final_smoothed.nii"
    output = process_file(file_path)
    
    for key in output.keys():
        print(f"{key}: {output[key]}", end=' ')
